...
O sistema SHALL expor `GET /leads` retornando todos os leads (sem filtro por criador) com detalhes completos: campos principais, `contacts` e `comments` (ordenados por `created_at` ASC) de cada lead. O sistema SHALL buscar contatos e comentários em batch (uma query por tabela), não em N+1.

#### Scenario: Listagem retorna todos os leads com contatos e comentários

- **WHEN** `GET /leads` por usuário autorizado
- **THEN** o sistema SHALL retornar HTTP 200 com array de leads contendo `id`, `company_name`, `status`, `created_by`, `created_at`, `updated_at`, campos de endereço, `contacts` (array) e `comments` (array)

### Requirement: Criação de lead

O sistema SHALL expor `POST /leads`. O campo `created_by` SHALL ser preenchido automaticamente com o `id` do caller. O campo `interest_items` é opcional e SHALL aceitar um array de strings correspondentes a nomes de itens ativos em `portfolio_items`. O service SHALL validar que cada nome existe em `portfolio_items WHERE deleted_at IS NULL` antes de salvar.

#### Scenario: Criação com dados mínimos

- **WHEN** `POST /leads` com `company_name`, `address_*` (todos obrigatórios exceto `complemento`) e `status` omitido
- **THEN** o sistema SHALL retornar HTTP 201 com o lead criado, `status: 'nao_contatado'` e `interest_items: []`

#### Scenario: Criação com serviços de interesse válidos

- **WHEN** `POST /leads` com `interest_items: ["Consultoria Energética", "Auditoria Elétrica"]` onde ambos existem em `portfolio_items` ativos
...
O sistema SHALL expor `GET /leads/:id` retornando o lead com seus contatos, `interest_items` e comentários (ordenados por `created_at` ASC).

#### Scenario: Lead encontrado

- **WHEN** `GET /leads/:id` com UUID existente
- **THEN** o sistema SHALL retornar HTTP 200 com lead completo incluindo arrays `contacts`, `interest_items` e `comments`
...
O sistema SHALL expor `PATCH /leads/:id` aceitando qualquer subconjunto dos campos do lead (exceto `created_by` e `created_at`). O campo `interest_items`, se fornecido, SHALL substituir completamente o array existente. O service SHALL validar que cada item do novo array existe em `portfolio_items WHERE deleted_at IS NULL`.
