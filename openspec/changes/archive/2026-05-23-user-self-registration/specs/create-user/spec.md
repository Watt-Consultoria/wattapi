...
O `id` e o `email` do novo usuário são extraídos do JWT (`sub` e `email`) — não do body. O `role` é sempre `'consultor'`, independentemente de qualquer dado enviado pelo cliente.

#### Scenario: Auto-registro bem-sucedido
- **WHEN** `jwtStatus = 'user-not-found'` e o body contém `{ name, sector, cpf }` válidos
- **THEN** o sistema SHALL inserir uma nova row com `id = jwtClaims.sub`, `email = jwtClaims.email`, `role = 'consultor'` e os campos do body, e retornar HTTP 201 com o objeto do usuário criado
