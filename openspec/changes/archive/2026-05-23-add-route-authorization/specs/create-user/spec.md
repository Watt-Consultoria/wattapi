...
Only authenticated users can call this endpoint. The `email` field in the request body MUST match the authenticated caller's email — no user may create a record with a different email, regardless of role.

#### Scenario: Successful user creation with own email
- **WHEN** an authenticated caller sends `POST /users` with a valid JSON body where `email` matches `caller.email`
...
#### Scenario: Creation with different email is rejected
- **WHEN** an authenticated caller sends `POST /users` with an `email` that differs from `caller.email`
